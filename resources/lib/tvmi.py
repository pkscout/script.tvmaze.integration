...
def _mark_one(show_info, mark_type, add_followed, tvmcache, tvmcachefile, tvmazeapi, lw):
    ...
    if show_info:
        lw.log(
            ['show info found, trying to match with cached TV Maze information first'])
        ...
        if not tvmazeid:
            lw.log(
                ['no match, getting updated followed shows from TV Maze and trying again'])
            if add_followed:
                showname = show_info['name']
            else:
                showname = ''
            tvmcache = _update_followed_cache(
                tvmcachefile, tvmazeapi, lw, showname=showname)
            tvmazeid = _match_from_followed_shows(show_info, tvmcache, lw)
        if tvmazeid:
            lw.log(['found tvmazeid of %s' %
                   tvmazeid, 'attempting to get episode id'])
            params = {'season': show_info['season'],
                      'number': show_info['episode']}
            success, loglines, results = tvmazeapi.getEpisodeBySeasonEpNumber(
                tvmazeid, params)
            lw.log(loglines)
            if not success:
            ...
            try:
                ...
                episodeid = ''
            if episodeid:
                lw.log(['got back episode id of %s' %
                       episodeid, 'marking episode on TV Maze'])
                success, loglines, results = tvmazeapi.markEpisode(
                    episodeid, marked_as=mark_type)
                lw.log(loglines)
                if not success:
                    lw.log(
                        ['no valid response returned from TV Maze, show was not marked'])
            else:
...
def _match_from_followed_shows(show_info, tvmcache, lw):
    ...
    for followed_show in tvmcache:
        ...
        lw.log(['checking for %s matching %s' %
               (show_info['name'], followed_name)])
...
def _update_followed_cache(tvmcachefile, tvmazeapi, lw, showname=''):
    ...
    success, loglines, results = tvmazeapi.getFollowedShows(
        params={'embed': 'show'})
    lw.log(loglines)
    if success:
        tvmcache = results
    ...
    if tvmcache:
        success, loglines = writeFile(
            json.dumps(tvmcache), tvmcachefile, wtype='w')
        lw.log(loglines)
    return tvmcache


def _get_json(method, params, lw):
    json_call = '{"jsonrpc":"2.0", "method":"%s", "params":%s, "id":1}' % (
        method, params)
...
def _startup(lw, settings, dialog):
    ...
    if not (settings['tvmaze_user'] and settings['tvmaze_apikey']):
        dialog.ok(settings['ADDONLANGUAGE'](32200),
                  settings['ADDONLANGUAGE'](32301))
...
class tvmContext:
    ...
    def _init_vars(self):
        ...
        self.LW = Logger(preamble='[TVMI Context]',
                         logdebug=self.SETTINGS['debug'])
        self.LW.log(['loaded settings', _logsafe_settings(self.SETTINGS)])
        self.TVMAZE = tvmaze.API(
            user=self.SETTINGS['tvmaze_user'], apikey=self.SETTINGS['tvmaze_apikey'])
        self.DIALOG = xbmcgui.Dialog()
        self.KODIMONITOR = xbmc.Monitor()
        self.TVMCACHEFILE = os.path.join(
            self.SETTINGS['ADDONDATAPATH'], 'tvm_followed_cache.json')
    ...
    def _get_details_from_path(self, thepath):
        ...
        epid = re.findall(
            '\/titles\/[0-9].*[0-9]\/.*\/([0-9].*[0-9])\?season', thepath)[0]
        ...
        self.LW.log(['returning showname of %s, season of %s, and episode of %s' % (
            showname, str(season), str(episode))])
        return {'name': showname, 'season': season, 'episode': episode}

    def _manage_show_follow(self, action):
        with busyDialog():
            _manage_followed(sys.listitem.getLabel(),
                             action, self.TVMAZE, self.LW)

    def _manage_show_tag(self, action):
        with busyDialog():
            taglist, tagmap = _build_tag_list(self.TVMAZE, self.LW)
        ret = self.DIALOG.select(
            self.SETTINGS['ADDONLANGUAGE'](32204), taglist)
        if ret == -1:
            return
        tagid = tagmap[taglist[ret]]
        with busyDialog():
            showid = _manage_followed(re.sub(
                r' \([0-9]{4}\)', '', sys.listitem.getLabel()), 'follow', self.TVMAZE, self.LW)
            if action == 'tag':
                success, loglines, result = self.TVMAZE.tagShow(showid, tagid)
            else:
                success, loglines, result = self.TVMAZE.unTagShow(
                    showid, tagid)
    ...
    def _manage_show_mark(self, action):
        ...
        if action == 'mark_watched':
            ...
            mark_type = -1
        with busyDialog():
            _mark_one(show_info, mark_type, self.SETTINGS['add_followed'], [
            ], self.TVMCACHEFILE, self.TVMAZE, self.LW)
...
class tvmManual:
    ...
    def __init__(self):
        ...
        ret = self.DIALOG.select(
            self.SETTINGS['ADDONLANGUAGE'](32201), options)
    ...
    def _init_vars(self):
        self.SETTINGS = loadSettings()
        self.LW = Logger(preamble='[TVMI Manual]',
                         logdebug=self.SETTINGS['debug'])
        self.LW.log(['loaded settings', _logsafe_settings(self.SETTINGS)])
        self.TVMAZE = tvmaze.API(
            user=self.SETTINGS['tvmaze_user'], apikey=self.SETTINGS['tvmaze_apikey'])
    ...
    def _option_follow_shows(self):
        ...
        ret = self._select_shows_dialog(
            self.SETTINGS['ADDONLANGUAGE'](32202), showlist)
        if not ret:
            return
        else:
            with busyDialog():
    ...
    def _option_unfollow_shows(self):
        ...
        ret = self._select_shows_dialog(
            self.SETTINGS['ADDONLANGUAGE'](32203), followedlist)
        if not ret:
            return
        else:
            with busyDialog():
    ...
    def _option_tag_shows(self):
        ...
        showlist = self._build_show_list()
        ret = self._select_shows_dialog(
            self.SETTINGS['ADDONLANGUAGE'](32205), showlist)
        if not ret:
            return
        else:
            with busyDialog():
    ...
    def _option_untag_shows(self):
        ...
        ret = self._select_shows_dialog(
            self.SETTINGS['ADDONLANGUAGE'](32206), taggedlist)
        if not ret:
            return
        else:
            with busyDialog():
    ...
    def _build_show_list(self):
        ...
        items = sorted(_get_json(method, params, self.LW).get(
            'tvshows', []), key=lambda x: x['label'])
        for item in items:
    ...
    def _build_tvmaze_list(self, tagid=''):
        ...
        tvmazemap = {}
        if tagid:
            success, loglines, results = self.TVMAZE.getTaggedShows(
                tagid, params={'embed': 'show'})
        else:
            results = _update_followed_cache(
                self.TVMCACHEFILE, self.TVMAZE, self.LW)
        if not results:
            return [], {}
    ...
    def _add_shows(self, showchoices, showlist, tagid=''):
        ...
        for showchoice in showchoices:
            if showchoice == 0:
                continue
            showid = _manage_followed(re.sub(
                r' \([0-9]{4}\)', '', showlist[showchoice]), 'follow', self.TVMAZE, self.LW)
            if showid and tagid:
    ...
    def _unfollow_shows(self, showchoices, showlist, showmap, tagid=''):
        ...
        for showchoice in showchoices:
            ...
            if tagid:
                success, loglines, result = self.TVMAZE.unTagShow(
                    showmap.get(showlist[showchoice], 0), tagid)
            else:
                success, loglines, result = self.TVMAZE.unFollowShow(
                    showmap.get(showlist[showchoice], 0))
            self.LW.log(loglines)
            self.KODIMONITOR.waitForAbort(0.12)
...
class tvmMonitor(xbmc.Monitor):
    ...
    def onNotification(self, sender, method, data):
        if 'Player.OnPlay' in method:
            ...
            if self.KODIPLAYER.isPlayingVideo():
                ...
                is_an_episode = data.get('item', {}).get(
                    'type', '') == 'episode'
                if is_an_episode:
            ...
            if self.PLAYINGEPISODE:
                ...
                self.LW.log(['MONITOR METHOD: %s DATA: %s' %
                            (str(method), str(data))])
                played_percentage = (
                    self.PLAYINGEPISODETIME / self.PLAYINGEPISODETOTALTIME) * 100
                self.LW.log(['got played percentage of %s' %
                            str(played_percentage)], xbmc.LOGINFO)
                if played_percentage >= float(self.SETTINGS['percent_watched']):
                    self.LW.log(
                        ['item was played for the minimum percentage in settings, trying to mark'], xbmc.LOGINFO)
                    self._mark_episodes('playing')
                else:
                    self.LW.log(
                        ['item was not played long enough to be marked, skipping'], xbmc.LOGINFO)
                self._reset_playing()
    ...
    def _init_vars(self):
        self.SETTINGS = loadSettings()
        self.LW = Logger(preamble='[TVMI Monitor]',
                         logdebug=self.SETTINGS['debug'])
        self.LW.log(['the settings are:', _logsafe_settings(self.SETTINGS)])
        self._set_property('script.tvmi.hidemenu', str(
            self.SETTINGS['hidemenu']).lower())
        self.TVMCACHEFILE = os.path.join(
            self.SETTINGS['ADDONDATAPATH'], 'tvm_followed_cache.json')
        self.EPISODECACHE = os.path.join(
            self.SETTINGS['ADDONDATAPATH'], 'episode_cache.json')
        self.KODIPLAYER = xbmc.Player()
        self.TVMAZE = tvmaze.API(
            user=self.SETTINGS['tvmaze_user'], apikey=self.SETTINGS['tvmaze_apikey'])
        self.TVMCACHE = _update_followed_cache(
            self.TVMCACHEFILE, self.TVMAZE, self.LW)
        self.REMOVEDITEMS = []
        self._reset_playing()
        self._reset_scanned()
    ...
    def _get_show_ep_info(self, thetype, data):
        ...
        if epid:
            method = 'VideoLibrary.GetEpisodeDetails'
            params = '{"episodeid":%s, "properties":["season", "episode", "tvshowid"]}' % str(
                epid)
            ...
            self.LW.log(['moving on with season of %s, episode of %s, and showid of %s' % (
                str(season), str(episode), str(showid))])
            if showid:
                method = 'VideoLibrary.GetTVShowDetails'
                params = '{"tvshowid":%s}' % str(showid)
                r_dict = _get_json(method, params, self.LW)
    ...
    def _update_episode_cache(self, epid=None, item=None, items=None):
        ...
        if epid:
            ...
            for item in items:
                epcache_json[str(item['epid'])] = item
        if cache_changed:
            success, loglines = writeFile(json.dumps(
                epcache_json), self.EPISODECACHE, 'w')
            self.LW.log(loglines)
    ...
    def _mark_episodes(self, thetype):
        ...
        for item in items:
            ...
            self.TVMCACHE = _mark_one(
                item, mark_type, self.SETTINGS['add_followed'], self.TVMCACHE, self.TVMCACHEFILE, self.TVMAZE, self.LW)
            if self.abortRequested():
                break
